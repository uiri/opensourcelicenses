...
Another major change introduced with GPLv3 is that laws that criminalized the development of free software under GPLv2 no longer apply. This includes legations such as the Digital Millennium Copyright Act and the European Union Copyright Directive.
...
Finally, GPLv3 aims to avoid discriminatory patent deals. In the past companies like Microsoft have claimed that they will not sue free software users for patent infringment if they obtain their software from a vendor that pays Microsoft. \cite{gpl3}

\newpage

\subsection{GNU GPL Variations}

In addition to the main GPL license, two subsets exist which address specific problems related to the the primary GPL license.
...
The GNU Affero General Public License was designed to close a perceived application service provider loophole in the original GPL. It includes an additional provision which requires that the complete source code be made available to any network user of the AGPL licensed work.

\section{MPL}

The Mozilla Public License is a license that aims at finding a balance between the BSD and GPL licenses. It is includes in various open source projects including; Thunderbird, Firefox, and Libreoffice.
